<script>
  ...
  if(blogcommentId !== 0 ){
    ...
    const res = await fetch(`${serverUrl}/getcomments/${blogcommentId}`,{credentials:'include'})
    if(res.ok){
  ...
  async function addComments() {
    if(commentText !== ''){
      const res = await fetch(`${serverUrl}/newcomment`,{
      method:'post',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({blogcommentId , commentText }),
      credentials:'include'
    })
    if(res.ok){
      commentText = ''  
      ...
      alert('Comment input cant be empty')
    }
  }
...
<div class="card">
      ...
      <div class="comment-container">
        <div class="user">
          <div class="user-pic">
            ...
            <img src={comment.commenter_pic}  alt="Profile picture" width="40px" height="40px" class="rounded-circle">
          </div>
          <div class="user-info">
            <span>{comment.commenter_username}</span>
  ...
  <div class="text-box">
    {#if !logged_in}
       <center><span style="color: orangered;">Please login to be able to add comment</span></center>
         {:else}
         <form on:submit|preventDefault={addComments}>
          <div class="box-container">
            <textarea bind:value={commentText} placeholder="comment"></textarea>
            <div>
